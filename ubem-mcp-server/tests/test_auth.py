"""Tests for ubem_mcp_server.auth — ContextVar and middleware."""
import asyncio
import pytest


def test_current_token_label_default_is_none():
    from ubem_mcp_server.auth import current_token_label
    assert current_token_label() is None


def test_current_token_label_set_and_read_in_same_context():
    from ubem_mcp_server.auth import _token_label, current_token_label
...
def _build_app(tokens):
    ...
    from ubem_mcp_server.auth import AuthMiddleware, current_token_label
...
async def test_build_app_health_and_auth_path(monkeypatch):
    ...
    from ubem_mcp_server.config import reload_config
    cfg = reload_config()
