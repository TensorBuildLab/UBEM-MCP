...
class PathConfig:
    """Path configuration"""
    workspace_root: str = "/workspace/ubem-mcp-server"
    sample_files_path: str = ""
    temp_dir: str = "/tmp"
    output_dir: str = "/workspace/ubem-mcp-server/outputs"
...
@dataclass
class BatchConfig:
    """Batch-simulation configuration (portfolio/multi-model runs)"""
    max_workers: int = 0  # 0 = resolved to os.cpu_count() in __post_init__
    batch_output_dir: str = ""  # resolved relative to paths.output_dir if unset
...
class Config:
    ...
    def _setup_batch(self):
        """Set up batch-simulation defaults from environment variables"""
        max_workers_str = os.getenv("UBEM_BATCH_MAX_WORKERS")
        if max_workers_str:
            try:
                self.batch.max_workers = int(max_workers_str)
            except ValueError:
                raise ValueError(
                    f"UBEM_BATCH_MAX_WORKERS must be an integer, got: {max_workers_str!r}"
                )
        if not self.batch.max_workers or self.batch.max_workers < 1:
            self.batch.max_workers = os.cpu_count() or 4

        if not self.batch.batch_output_dir:
            self.batch.batch_output_dir = str(Path(self.paths.output_dir) / "batches")
    ...
    def _validate_config(self):
        ...
        # Create batch output directory if it doesn't exist (tolerant in dev/test)
        try:
            os.makedirs(self.batch.batch_output_dir, exist_ok=True)
        except OSError:
            logger.warning("Could not create batch output dir %s", self.batch.batch_output_dir)
